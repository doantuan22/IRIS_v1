...
class _ExpertDetailPageState extends State<ExpertDetailPage> {
  ...
  void initState() {
    ...
    if (section != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) => _scrollToSection(section));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final mocPhatTrien = chunks.where((c) => c.phanLoai == 'moc_phat_trien').toList();
    final dauHieuLuuY = chunks.where((c) => c.phanLoai == 'dau_hieu_luu_y').toList();
    final giaiThich = chunks.where((c) => c.phanLoai == 'giai_thich').toList();

    return Scaffold(
      appBar: AppBar(title: Text('${widget.linhVucLabel} — Giải thích chuyên môn')),
      body: chunks.isEmpty
          ? const Center(
              child: Padding(
          ...
          : ListView(
              ...
              children: [
                ...
                if (mocPhatTrien.isNotEmpty) ...[
                  _ExpertSection(key: _mocPhatTrienKey, title: 'Mốc phát triển', items: mocPhatTrien),
                  const SizedBox(height: 20),
                ],
                if (dauHieuLuuY.isNotEmpty) ...[
                  _ExpertSection(key: _dauHieuLuuYKey, title: 'Dấu hiệu cần lưu ý', items: dauHieuLuuY),
                  const SizedBox(height: 20),
                ],
                if (giaiThich.isNotEmpty) ...[
                  _ExpertSection(key: _giaiThichKey, title: 'Giải thích chuyên môn', items: giaiThich),
                  const SizedBox(height: 20),
                ],
                Container(
