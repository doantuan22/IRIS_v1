...
/// Script ingest 196 entry dữ liệu tham khảo So sánh dải tuổi 24-47 tháng
/// vào SQLite database `expert_knowledge_chunks` sử dụng NVIDIA NIM Embedding API.
///
/// Chạy:
///   dart run scripts/ingest_so_sanh_24_47.dart
void main(List<String> args) async {
  print('=== INGEST DỮ LIỆU SO SÁNH 24-47 THÁNG ===');
  ...
  final jsonFile = File('so_sanh_24_47_thang.json');
  if (!jsonFile.existsSync()) {
    print('LỖI: Không tìm thấy file so_sanh_24_47_thang.json!');
    exit(1);
  }
  ...
  print('Đã đọc ${entries.length} entry từ so_sanh_24_47_thang.json.');

  final dbPath = args.isNotEmpty ? args.first : 'iris_so_sanh_24_47.db';
  ...
  // Xoá sạch toàn bộ dữ liệu dải 24-47 cũ (nếu có)
  final deleted = await db.delete(
    'expert_knowledge_chunks',
    where: "content_type = 'so_sanh' AND do_tuoi_thang_min = 24 AND do_tuoi_thang_max = 47",
  );
  print('Đã xoá $deleted dòng dải 24-47 cũ trước khi insert.');
  ...
  final countRows = await db.rawQuery(
    "SELECT COUNT(*) as count FROM expert_knowledge_chunks WHERE content_type='so_sanh' AND do_tuoi_thang_min=24 AND do_tuoi_thang_max=47",
  );
  final count = countRows.first['count'];
  print('Tổng số dòng dải 24-47 tháng trong DB: $count');
