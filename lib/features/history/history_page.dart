...
class _HistoryPageState extends State<HistoryPage> {
  ...
  IconData _iconFor(String eventType) => switch (eventType) {
    'sang_loc' => Icons.fact_check_rounded,
    'danh_gia' => Icons.checklist_rounded,
    'video' => Icons.videocam_rounded,
    'ho_so' => Icons.person_rounded,
    'tong_quan' => Icons.auto_awesome_rounded,
    _ => Icons.event_note_rounded,
  };

  Color _colorFor(String eventType) => switch (eventType) {
    'sang_loc' => IrisColors.primary,
    'danh_gia' => IrisColors.social,
    'video' => IrisColors.emotion,
    'ho_so' => IrisColors.language,
    'tong_quan' => IrisColors.warning,
    _ => IrisColors.neutral,
  };
  ...
  List<MapEntry<DateTime, List<HistoryLog>>> _groupByDay(
        ...
        builder: (context, snapshot) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return Padding(
                padding: const EdgeInsets.only(bottom: IrisSpacing.md),
                child: Column(
                    ...
                    Card(
                      child: Column(
                        children: group.value
                            .map(
                              (log) => ListTile(
                                leading: IrisIconChip(
                                  icon: _iconFor(log.eventType),
                                  color: _colorFor(log.eventType),
                                ),
                                title: Text(
                                  log.description ?? _labelFor(log.eventType),
                                ),
                                subtitle: Text(
                                  '${_labelFor(log.eventType)} • ${_formatTime(log.eventDate)}',
                                ),
                              ),
                            )
                            .toList(),
