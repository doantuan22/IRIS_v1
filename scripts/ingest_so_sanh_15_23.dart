...
/// Script ingest 200 entry dữ liệu tham khảo So sánh dải tuổi 15-23 tháng
/// vào SQLite database `expert_knowledge_chunks` sử dụng NVIDIA NIM Embedding API.
///
/// Chạy:
///   dart run scripts/ingest_so_sanh_15_23.dart
void main(List<String> args) async {
  print('=== INGEST DỮ LIỆU SO SÁNH 15-23 THÁNG ===');
  ...
  final jsonFile = File('so_sanh_15_23_thang.json');
  if (!jsonFile.existsSync()) {
    print('LỖI: Không tìm thấy file so_sanh_15_23_thang.json!');
    exit(1);
  }
  ...
  print('Đã đọc ${entries.length} entry từ so_sanh_15_23_thang.json.');

  final dbPath = args.isNotEmpty ? args.first : 'iris_so_sanh_15_23.db';
  ...
  // Xoá các placeholder cũ dải 15-23 (nếu có)
  await db.execute('''
    DELETE FROM expert_knowledge_chunks 
    WHERE content_type = 'so_sanh' 
      AND do_tuoi_thang_min = 15 
      AND do_tuoi_thang_max = 23
      AND content LIKE '%[Placeholder%'
  ''');
  ...
  final countRows = await db.rawQuery(
    "SELECT COUNT(*) as count FROM expert_knowledge_chunks WHERE content_type='so_sanh' AND do_tuoi_thang_min=15 AND do_tuoi_thang_max=23",
  );
  final count = countRows.first['count'];
  print('Tổng số dòng dải 15-23 tháng trong DB: $count');

  final byDomain = await db.rawQuery('''
    SELECT linh_vuc, phan_loai, COUNT(*) as count 
    FROM expert_knowledge_chunks 
    WHERE content_type='so_sanh' AND do_tuoi_thang_min=15 AND do_tuoi_thang_max=23
